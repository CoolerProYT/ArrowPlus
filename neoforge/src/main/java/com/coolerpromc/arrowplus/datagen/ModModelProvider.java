...
public class ModModelProvider extends ModelProvider {
    ...
    private void generateArrow(ItemModelGenerators itemModels, Item item) {
        TextureMapping baseMapping = new TextureMapping()
                .put(TextureSlot.LAYER0, new Material(Constants.id("item/arrow_stick")))
                .put(TextureSlot.LAYER1, new Material(Constants.id("item/arrow_head")))
                .put(TextureSlot.LAYER2, new Material(Constants.id("item/arrow_feather")));

        TextureMapping tippedMapping = new TextureMapping()
                .put(TextureSlot.LAYER0, new Material(Constants.id("item/arrow_stick")))
                .put(TextureSlot.LAYER1, new Material(Constants.id("item/arrow_head")))
                .put(TextureSlot.LAYER2, new Material(Constants.id("item/arrow_feather")))
                .put(LAYER3, new Material(Constants.id("item/arrow_tipped")));
    ...
    private Identifier getModelLocation(Item item, String suffix) {
        return Constants.id("item/" + BuiltInRegistries.ITEM.getKey(item).getPath() + suffix);
    }
}
