...
public class ModArrowEntity extends AbstractArrow {
    ...
    protected void doPostHurtEffects(LivingEntity livingEntity) {
        ...
        getArrowData().effects().forEach((resourceLocation, integer) -> BuiltInRegistries.POTION.getOptional(resourceLocation).ifPresent(potionReference -> potionReference.getEffects().forEach(instance -> livingEntity.addEffect(
        ...
        if (potioncontents.potion().isPresent()) {
            for(MobEffectInstance mobeffectinstance : ((Potion)((Holder<?>)potioncontents.potion().get()).value()).getEffects()) {
                livingEntity.addEffect(new MobEffectInstance(mobeffectinstance.getEffect(), Math.max(mobeffectinstance.mapDuration((p_268168_) -> p_268168_ / 8), 1), mobeffectinstance.getAmplifier(), mobeffectinstance.isAmbient(), mobeffectinstance.isVisible()), entity);
            }
        }
