    ...
    public static final ItemGroup ARROW_PLUS_TAB = Registry.register(Registries.ITEM_GROUP, Identifier.of(ArrowPlus.MODID, "arrow_plus"),
            FabricItemGroup.builder().icon(() -> new ItemStack(Items.ARROW))
                    ...
                    .entries((itemDisplayParameters, output) -> {
                        List<RegistryEntry.Reference<ArrowData>> holder = itemDisplayParameters.lookup().getOrThrow(ModRegistries.ARROW_DATA_KEY).streamEntries().filter(reference -> !ArrowPlusConfig.CONFIG.getRemoval().contains(reference.registryKey().getValue().getPath())).toList();
                        ...
                        holder.forEach(arrowData -> itemDisplayParameters.lookup().getOrThrow(RegistryKeys.POTION).streamEntries().forEach(potionReference -> {
