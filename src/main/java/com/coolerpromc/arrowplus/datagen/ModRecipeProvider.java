...
public class ModRecipeProvider extends RecipeProvider {
    ...
    protected void buildRecipes(RecipeOutput output) {
        ...
        stickRecipe(Items.COPPER_INGOT, ModItems.COPPER_STICK.get(), output);
        stickRecipe(Items.IRON_INGOT, ModItems.IRON_STICK.get(), output);
        stickRecipe(Items.GOLD_INGOT, ModItems.GOLD_STICK.get(), output);
        stickRecipe(Items.DIAMOND, ModItems.DIAMOND_STICK.get(), output);
        stickRecipe(Items.EMERALD, ModItems.EMERALD_STICK.get(), output);
        stickRecipe(Items.NETHERITE_INGOT, ModItems.NETHERITE_STICK.get(), output);

        ShapedRecipeBuilder.shaped(RecipeCategory.MISC, ModItems.GILDED_FEATHER.get(), 4)
                .pattern(" G ")
                .pattern("GFG")
                .pattern(" G ")
        ...
        lookupProvider.lookupOrThrow(ModRegistries.ARROW_DATA_KEY).listElements().forEach(holder -> {
            ...
            if (arrowData.material().left().isPresent()){
                ...
                ingredient = Ingredient.of(arrowData.material().right().get());
    ...
    private void stickRecipe(Item item, ModStickItem stickItem, RecipeOutput output){
        ShapedRecipeBuilder.shaped(RecipeCategory.MISC, stickItem, 4)
                .pattern("S")
                .pattern("S")
