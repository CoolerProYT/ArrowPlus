...
public class ArrowRecipe extends CustomRecipe {
    ...
    public boolean matches(CraftingInput craftingInput, Level level) {
        if (craftingInput.width() == 1 && craftingInput.height() == 3 && craftingInput.ingredientCount() == 3){
            ...
            List<ResourceLocation> materialList = new ArrayList<>();
            ...
            if (materialList.contains(BuiltInRegistries.ITEM.getKey(materialStack.getItem()))) {
                hasMaterial = true;
            }
    ...
    public ItemStack assemble(CraftingInput craftingInput, HolderLookup.Provider provider) {
        ...
        provider.lookupOrThrow(ModRegistries.ARROW_DATA_KEY).listElements().forEach(holder ->{
            ...
            if (holder.value().material().equals(BuiltInRegistries.ITEM.getKey(materialStack.getItem()))) {
