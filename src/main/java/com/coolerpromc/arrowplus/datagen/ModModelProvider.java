...
public class ModModelProvider extends ModelProvider {
    ...
    protected void registerModels(BlockModelGenerators blockModels, ItemModelGenerators itemModels) {
        itemModels.generateItemWithTintedOverlay(ModItems.ARROW_PLUS.get(), "_layer1", new ArrowTintSource(0xFFFFFFFF));
    ...
    private ResourceLocation createLayeredItemModel(ItemModelGenerators itemModels, Item item, String suffix) {
        return ModelTemplates.TWO_LAYERED_ITEM.create(TextureMapping.getItemTexture(item, suffix), TextureMapping.layered(getModelLocation(item, suffix), getModelLocation(item, suffix + "_head")), itemModels.modelOutput);
    }
