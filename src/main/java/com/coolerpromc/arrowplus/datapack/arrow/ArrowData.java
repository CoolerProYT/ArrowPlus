...
public record ArrowData(HolderSet<Item> material, double baseDamage, int color, String translationKey, boolean flame, double gravity, Map<ResourceLocation, Integer> effects, Holder<Item> feather, Optional<Holder<FeatherData>> featherData, Holder<Item> stick, Optional<Holder<StickData>> stickData, int outputAmount) {
    ...
    public static final Codec<ArrowData> CODEC = RecordCodecBuilder.<ArrowData>create(instance -> instance.group(
            Codecs.ITEM_HOLDER_SET_CODEC.fieldOf("material").forGetter(ArrowData::material),
    ...
    public static final ArrowData EMPTY = ArrowData.builder(Items.AIR).translationKey("item.arrowplus.cheated_item").build();

    public static Builder builder(Item material) {
        return new Builder(HolderSet.direct(material.builtInRegistryHolder()));
    }

    public static Builder builder(TagKey<Item> material) {
        return new Builder(BuiltInRegistries.ITEM.getOrCreateTag(material));
    }

    public static Builder builder(HolderSet<Item> material) {
        return new Builder(material);
    }

    public static class Builder {
        private final HolderSet<Item> material;
        private double baseDamage = 2.0;
        private int color = 0xFFFFFFFF;
        private String translationKey = "";
        private boolean flame = false;
        private double gravity = 0.05;
        private Map<ResourceLocation, Integer> effects = Map.of();
        ...
        private Builder(HolderSet<Item> material) {
        ...
        public Builder effects(Map<ResourceLocation, Integer> effects) {
    ...
    public boolean isValidMaterial(ItemStack materialStack, ItemStack stickStack, ItemStack featherStack){
        return material.stream().anyMatch(h -> materialStack.is(material)) && stickStack.is(stick) && isValidFeather(featherStack);
    }
