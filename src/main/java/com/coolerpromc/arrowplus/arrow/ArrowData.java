...
public record ArrowData(Either<Holder<Item>, TagKey<Item>> material, double baseDamage, int color, String translationKey, boolean flame, double gravity, Map<ResourceLocation, Integer> effects, Holder<Item> feather, Holder<Item> stick, int outputAmount) {
    ...
    public static final Codec<ArrowData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            ITEM_HOLDER_CODEC.fieldOf("feather").validate(itemHolder -> itemHolder.is(Items.FEATHER.builtInRegistryHolder()) || itemHolder.value() instanceof ModFeatherItem ? DataResult.success(itemHolder) : DataResult.error(() -> "Item must be vanilla feather or feather from Arrow+ mod.")).forGetter(ArrowData::feather),
            ITEM_HOLDER_CODEC.fieldOf("stick").validate(itemHolder -> itemHolder.is(Items.STICK.builtInRegistryHolder()) || itemHolder.value() instanceof ModStickItem ? DataResult.success(itemHolder) : DataResult.error(() -> "Item must be vanilla stick or stick from Arrow+ mod.")).forGetter(ArrowData::stick),
