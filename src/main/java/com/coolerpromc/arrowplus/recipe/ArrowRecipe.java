...
public class ArrowRecipe extends SpecialCraftingRecipe {
    ...
    public boolean matches(CraftingRecipeInput craftingInput, World level) {
        if (craftingInput.getWidth() == 1 && craftingInput.getHeight() == 3 && craftingInput.getStackCount() == 3){
            List<RegistryEntry.Reference<ArrowData>> arrowDataList = level.getRegistryManager().getWrapperOrThrow(ModRegistries.ARROW_DATA_KEY).streamEntries().filter(reference -> !ArrowPlusConfig.CONFIG.getRemoval().contains(reference.registryKey().getValue().getPath())).toList();
    ...
    public ItemStack craft(CraftingRecipeInput craftingInput, RegistryWrapper.WrapperLookup provider) {
        ...
        List<RegistryEntry.Reference<ArrowData>> arrowDataList = provider.getWrapperOrThrow(ModRegistries.ARROW_DATA_KEY).streamEntries().filter(reference -> !ArrowPlusConfig.CONFIG.getRemoval().contains(reference.registryKey().getValue().getPath())).toList();
