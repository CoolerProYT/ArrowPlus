...
public class ArrowRecipe extends CustomRecipe {
    ...
    public ItemStack assemble(CraftingInput craftingInput) {
        List<Ingredient> materialList = new ArrayList<>();
        ...
        for (Holder<ArrowData> holder : arrowDataList){
            materialList.add(holder.value().ingredient());
            if (holder.value().ingredient().test(materialStack)) {
                arrowData = holder;
                break;
            }
        }

        if (arrowData != null && materialList.contains(arrowData.value().ingredient())){
